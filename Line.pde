...
//	float x1,y1,x2,y2;
	...
	PVector getPoint1() { return new PVector(x1,y1); }
	PVector getPoint2() { return new PVector(x2,y2); }
	...
	float Length(){ return mag(y2-y1, x2-x1); }
	float RadianAngle(){ return atan2(y2-y1, x2-x1); }
	...
	PVector ClosestSegmentPointToXY(float px, float py){
		...
		if (u < 0.0) pt = new PVector(x1, y1);
		else if (u > 1.0) pt = new PVector(x2, y2);
		else pt = new PVector(x1 + u * xd, y1 + u * yd);
