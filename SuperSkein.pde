...
//Page 1 GUI Widgets
VScrollbar layerHeight = new VScrollbar(0, 0, 10, AppHeight, 10); 
//AllPage GUI Widgets
	...
	if(GUIPage==1){
		...
		if(STLLoadedFlag){
			for(int i = STLFile.Triangles.size()-1;i>=0;i--){
				...
				Intersection = tri.GetZIntersect(MeshHeight*(1.-layerHeight.getPos()));
...
class STLLoadProc implements Runnable {
	public void run(){
		while(true){
			...
			else println("No STL File selected. Using "+STLName.Text);
